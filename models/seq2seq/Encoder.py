...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        # print(hidden)
        if self.model_type == "RNN":
            output, hidden = self.recurrent_layer(embeddings)
            #output (batch, seq len, hidden size)
            #hidden (1, batch, hidden, size)
            ...
            output, (hidden, _) = self.recurrent_layer(embeddings)
            #output (batch, seq len, hidden size)
            #hidden (1, batch, hidden, size)
            hidden = self.linear1(hidden)
            hidden = self.relu(hidden)
